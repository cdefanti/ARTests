...
namespace GoogleARCore.TextureReader
{
    ...
    public class EdgeDetector
    {
        ...
        private void HoughTransform(byte[] outputImage, IntPtr inputImage, int width, int height)
        {
            ...
            for (int j = 1; j < height - 1; j++)
            {
                for (int i = 1; i < width - 1; i++)
                {
                    ...
                    if (s_ImageBuffer[offset] == 0xFF)
                    {
                        for (int r = R_MIN; r < R_MAX; r++)
                        {
                            for (float t = 0f; t < 2f * Mathf.PI; t += T_STEP)
                            {
                                int x = (int)(i - r * Mathf.Cos(t));
                                int y = (int)(j - r * Mathf.Sin(t));
                            }
                        }
                    }
                }
